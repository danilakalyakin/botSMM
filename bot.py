...
def start(message):
    ...
    save_user(chat_id)  # сохраняем пользователя в users.json
...
def get_application_phone(message, name):
    ...
    msg = bot.send_message(chat_id, "✅ Ваша заявка принята! Мы вам перезвоним.", reply_markup=get_main_menu(chat_id))
    save_message(chat_id, msg.message_id)
